...
private Deque<Weblink> queue = new ArrayDeque<>();
		...
		public void run() {
			try {
				synchronized (weblink) {
					InputStream in = HttpConnectForInterruption.getInputStream(weblink.getUrl());
					
					// Background thread for stopping download process by closing the input stream
					...
					String htmlPage;
					try {
						htmlPage = HttpConnectForInterruption.download(in);						
				...
				synchronized (weblink) {
					...
					while (htmlPage == null) {
						try {
							...
							weblink.wait();
							//wait() method clears the interrupted status of thread and you surround wait(), sleep() {blocking methods}
							//with catch (InterruptedException e)
	...
	public void go() {
		...
		try {
			TimeUnit.SECONDS.sleep(1);
			//we are expecting the downloader thread to finish the job in 1 sec
		} catch (InterruptedException e) {
			e.printStackTrace();
		}
		...
		for (int i = 0; i < downloaderThreadList.size(); i++) {
			...
			if (downloaderThread.isAlive()) {
				//we can't interrupt downloader thread because it's using stream IO and that is uninterruptible
				//isAlive will return true if some downloader thread has not finished downloading and still at it.
